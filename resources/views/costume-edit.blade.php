...
<div>
    <form action="/costume-edit/{{ $cosplay->slug }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="mt-5 w-50">
        @if ($errors->any())
...
<div class="mb-3">
            ...
            <input type="text" name="cosplay_code" id="code" class="form-control" placeholder="Costume Code" value="{{ $cosplay->cosplay_code}}"> 
        </div>
        <div class="mb-3">
            <label for="title" class="form-label">Title</label>
            <input type="text" name="title" id="title" class="form-control" placeholder="Costume Title" value="{{ $cosplay->title }}">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            @if ($cosplay->cover!='')
            <img src="{{ asset('storage/cover/'.$cosplay->cover) }}" alt="" width="300px">
                @else 
...
<ul>
    @foreach ($cosplay->categories as $category)
        <li>{{ $category->name }}</li>
    @endforeach
</ul>
        </div>
